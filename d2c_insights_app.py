"""
D2C Insights — Streamlit App (Enhanced + Safe Version)
--------------------------------
...
- "What was my top selling product in June by net revenue?"
...
def normalize_colname(c: str):
    return (
        c.strip().lower().replace(" ", "_").replace("(", "").replace(")", "").replace(".", "").replace("/", "_")
    )
...
def generate_code(question: str, df: pd.DataFrame):
    ...
    prod_col = next((c for c in df.columns if any(k in c.lower() for k in ["short", "product", "sku", "name"])), df.columns[0])
    qty_col = next((c for c in df.columns if "qty" in c.lower()), None)
    net_rev_col = next((c for c in df.columns if "net" in c.lower() and "revenue" in c.lower()), None)
    gross_rev_col = next((c for c in df.columns if "gross" in c.lower() and "revenue" in c.lower()), None)
    gm_col = next((c for c in df.columns if "gm" in c.lower()), None)
    ret_qty_col = next((c for c in df.columns if "return" in c.lower() and "qty" in c.lower()), None)

    # Decide metric dynamically
    metric_col = qty_col  # default
    if any(k in q for k in ["net revenue", "netrevenue", "revenue after gst", "final revenue"]):
        metric_col = net_rev_col or gross_rev_col or qty_col
    elif any(k in q for k in ["gross revenue"]):
        metric_col = gross_rev_col or net_rev_col or qty_col
    elif any(k in q for k in ["gm", "margin", "profit"]):
        metric_col = gm_col or net_rev_col or qty_col
    elif any(k in q for k in ["qty", "quantity", "units sold"]):
        metric_col = qty_col

    # --- Top N by month ---
    for m in ["june", "july", "august", "september", "october"]:
        if "top" in q and ("sell" in q or "product" in q) and m in q:
            n = 10
            for i in range(5, 21):
                if f"top {i}" in q:
                    n = i
            ...
            return textwrap.dedent(f"""
                result = df[df['Month'].astype(str).str.lower().str.contains('{m}')].groupby('{prod_col}')[['{metric_col}']].sum().sort_values(by='{metric_col}', ascending=False).head({n})
            """)
    ...
    return "# Could not generate code for this question — try rephrasing or check your column names."
...
if uploaded:
    ...
    if df is None or df.empty:
        st.error("No data found in the uploaded file. Please check your file and try again.")
        st.stop()
    ...
    question = st.text_input("Type your question", value="What were my top 10 selling Products in June in terms of net revenue?")
